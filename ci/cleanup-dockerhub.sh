...
DOCKERHUB_NAMESPACE="${DOCKERHUB_NAMESPACE:-libops}"
...
registry_usage() {
    echo "  DOCKERHUB_USER       - Docker Hub username"
    echo "  DOCKERHUB_PASSWORD   - Docker Hub personal access token (PAT)"
    echo "  DOCKERHUB_NAMESPACE - Docker Hub namespace (default: libops)"
...
registry_validate_config() {
    if [ -z "$DOCKERHUB_USERNAME" ]; then
        echo -e "${RED}Error: DOCKERHUB_USER is not set${NC}"
        exit 1
    fi

    if [ -z "$DOCKERHUB_TOKEN" ]; then
        echo -e "${RED}Error: DOCKERHUB_PASSWORD is not set${NC}"
        exit 1
    fi
...
# Get preserved digests (main, latest, and semver tags, excluding arch-specific tags)
...
registry_get_preserved_digests() {
    ...
    echo "$all_versions" | jq -r '
        ...
        select(
            (.name == "main") or
            (.name == "latest") or
            (.name | test("^(main|latest)-")) or
            (.name | test("^nginx-[A-Za-z0-9_.-]+-php8[3-5]$")) or
            (
