...
usage() {
    echo "Usage:"
    echo "  $0 [directory]                                   # check for updates in-place"
    echo "  $0 <old_alpine_version> <new_alpine_version> [directory]   # migrate releases"
    echo ""
    echo "Examples:"
    echo "  $0                                  # update all Dockerfiles under ."
    echo "  $0 ./images                         # update all Dockerfiles under ./images"
    echo "  $0 alpine_3_22 alpine_3_24          # migrate 3.22 -> 3.24 under ."
    echo "  $0 alpine_3_22 alpine_3_24 ./images # migrate 3.22 -> 3.24 under ./images"
    echo ""
    echo "This script will:"
    echo "1. Find all Dockerfiles in the specified directory (current dir if not specified)"
    echo "2. In migrate mode, rewrite renovate comments from the old Alpine release to the new one"
    echo "3. Fetch the latest package versions from the Repology API"
    echo "4. Update the version numbers in the ARG declarations when they differ"
    echo ""
    echo "When no Alpine versions are given the script runs in \"check for updates\""
    echo "mode: it leaves each renovate comment's Alpine release as-is and only"
    echo "refreshes the pinned package versions."
    echo ""
    echo "Options:"
            ...
            if new_version=$(get_alpine_package_version "$package_name" "$target_alpine"); then
                ...
                if IFS= read -r next_line; then
                    if [[ "$next_line" =~ ^[[:space:]]*([A-Z_]+_VERSION)=([^[:space:]\\]*) ]]; then
                        ...
                        # Substitute only the version value so the original
                        # indentation and trailing "\" (absent on the last ARG)
                        # are preserved exactly.
                        local updated_version_line="${next_line/${current_version}/${new_version}}"
