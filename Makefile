...
.PHONY: test
## Runs docker compose tests for built images. Use IMAGE=<image> or TARGET=<image> to narrow the run.
test:
	@if [ -z "$(GO)" ]; then printf "Go is required to run tests.\n"; exit 127; fi
	$(GO) run ./cmd/buildkit test \
		$(if $(IMAGE),--image $(IMAGE),$(if $(TEST_IMAGE),--image $(TEST_IMAGE),)) \
		$(if $(TEST),--test $(TEST),) \
		--repository "$(REPOSITORY)" \
		--mode "$(TEST_MODE)" \
		--tag "$(firstword $(TAGS))" \
...
.PHONY: list-tests
## Lists docker compose tests selected by IMAGE=<image> or TARGET=<image>.
list-tests:
