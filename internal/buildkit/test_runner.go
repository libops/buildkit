...
func (r *Runner) selectTests() ([]TestCase, error) {
	...
	for _, image := range r.options.Images {
		...
		if r.metadata.KnownImage(image) {
			...
			for _, candidate := range r.metadata.Images {
				if imageContext(candidate) == image {
					imageFilter[candidate] = true
					matched = true
				}
			}
		}
	...
	for _, test := range all {
		...
		if testFilter[test.Image+"/"+test.Name] || testFilter[imageContext(test.Image)+"/"+test.Name] || testFilter[test.Name] {
			selected = append(selected, test)
		}
	}
	return selected, nil
...
func (r *Runner) discoverTests() ([]TestCase, error) {
	...
	for _, image := range r.metadata.Images {
		testsDir := filepath.Join(r.root, "images", imageContext(image), "tests")
...
func (r *Runner) runOne(test TestCase) testResult {
	...
	upResult := r.runComposeUp(test, env, spec, logDir, services)
...
func (r *Runner) runComposeUp(test TestCase, env []string, spec testSpec, logDir string, services []string) composeUpResult {
	...
	if spec.HealthCheck {
		if err := r.waitForHealthy(ctx, test, env, done, services); err != nil {
...
func (r *Runner) waitForHealthy(ctx context.Context, test TestCase, env []string, done <-chan struct{}, services []string) error {
	...
	for attempt := 1; attempt <= attempts; attempt++ {
		...
		select {
		case <-ctx.Done():
			return fmt.Errorf("no target service reported healthy before timeout: %w", ctx.Err())
		case <-done:
			return errors.New("docker compose up exited before any target service reported healthy")
		case <-ticker.C:
		}
	}
...
func (r *Runner) healthcheckServices(test TestCase, services []string) []string {
	candidates := []string{
		imageContext(test.Image),
		r.metadata.PublishedImage(test.Image),
	}
...
func (r *Runner) anyHealthyContainer(ctx context.Context, test TestCase, env []string, services []string) (bool, error) {
	var containers []string
	for _, service := range services {
		ps := r.composeOutput(ctx, test, env, "ps", "-q", service)
		if ps.Err != nil {
			return false, ps.Err
		}
		containers = append(containers, splitLines(ps.Output)...)
	}
